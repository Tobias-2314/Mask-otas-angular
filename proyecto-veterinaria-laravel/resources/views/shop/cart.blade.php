...
@section('contenido')
    ...
    <div id="guest-modal"
        class="fixed inset-0 z-[100] hidden items-center justify-center p-4 overflow-x-hidden overflow-y-auto"
        aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="fixed inset-0 transition-opacity bg-gray-900/80 backdrop-blur-sm" onclick="closeModal('guest-modal')">
        </div>
        ...
        <div class="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl transform transition-all p-8 sm:p-10">
            <div class="flex flex-col items-center">
                ...
                <p class="text-gray-600 text-center mb-8 font-medium">Para proceder con el pago y asegurar tus productos,
                    por favor inicia sesión o crea una cuenta.</p>

                <div class="flex flex-col w-full gap-4">
                    <a href="{{ route('login') }}"
                        class="flex items-center justify-center w-full py-4 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
                        <i class="fas fa-sign-in-alt mr-2"></i> Iniciar Sesión
                    </a>
                    <a href="{{ route('registro') }}"
                        class="flex items-center justify-center w-full py-4 px-6 bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold rounded-2xl transition-all duration-300">
                        <i class="fas fa-user-plus mr-2"></i> Registrarse
                    </a>
                    <button onclick="closeModal('guest-modal')"
                        class="mt-2 text-gray-400 hover:text-gray-600 text-sm font-semibold transition">
                        Continuar mirando
                    </button>
                </div>
            </div>
    ...
    <div id="success-modal"
        class="fixed inset-0 z-[100] hidden items-center justify-center p-4 overflow-x-hidden overflow-y-auto"
        aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="fixed inset-0 transition-opacity bg-gray-900/80 backdrop-blur-sm" onclick="closeModal('success-modal')">
        </div>
        ...
        <div class="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl transform transition-all overflow-hidden">
            <div class="p-8 sm:p-10">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-gray-600 text-center mb-8 font-medium text-lg">Tu compra se ha procesado
                        correctamente.<br>¡Gracias por elegir <strong>MASK!OTAS</strong>!</p>

                    <button onclick="closeModal('success-modal')"
                        class="flex items-center justify-center w-full py-4 px-6 bg-green-600 hover:bg-green-700 text-white font-bold rounded-2xl shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
                        Cerrar ahora
                    </button>
                </div>
            </div>
            ...
            <div class="absolute bottom-0 left-0 h-1.5 bg-green-100 w-full">
                <div id="auto-close-progress" class="h-full bg-green-500 transition-all duration-[5000ms] ease-linear w-0">
                </div>
            </div>
        </div>
    </div>
